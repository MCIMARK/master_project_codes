...
resultados =f'{drive}/modelo_iSAFE_ltbdm/'
datos = f'{drive}/recordings_iSAFE/'
...
def build_model(hp):
    ...
    '''capa de salida'''
    model.add(keras.layers.Dense(3, activation='softmax', name='output_layer_softmax'))
...
resultados =f'{drive}/modelo_iSAFE_std/'
datos = f'{drive}/recordings_iSAFE/'
...
def build_model(hp):
    ...
    model.add(Dropout(0.2,name=f'dropout_{i+1}'))
    '''capa de salida'''
    model.add(Dense(3, activation='softmax', name='output_layer_softmax'))
    model.compile(optimizer=Adam(0.001),loss='categorical_crossentropy',metrics=['accuracy'])
    global n
    n+=1
    print(f'modelo: {n}')
...
resultados =f'{drive}/modelo_iSAFE_schdlr/'
datos = f'{drive}/recordings_iSAFE/'
