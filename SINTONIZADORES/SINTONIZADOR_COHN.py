...
drive = 'D:/para_entrenamiendo'
resultados =f'{drive}/modelo_cohn_kanade_ltbdm/'
datos = f'{drive}/recordings_cohn_kanade/'
...
def build_model(hp):
    ...
    for i in range (hp.Int('hidden_layers',min_value = 1, max_value=3)):
        model.add(keras.layers.LSTM(hp.Choice(f'hidden_layer_{i}',values=[32,64]),dropout = 0.2,return_sequences=True,name=f'hidden_layer_{i}'))
...
resultados =f'{drive}/modelo_cohn_kanade_std/'
datos = f'{drive}/recordings_cohn_kanade/'
...
def build_model(hp):
    ...
    '''iteramos en un rango de 1 a 10 capas ocultas'''
    for i in range (hp.Int('hidden_layers',min_value = 1, max_value=3)):
        model.add(Bidirectional(LSTM(hp.Choice(f'hidden_layer_{i}',values=[32,64]),dropout = 0.2,return_sequences=True),name=f'hidden_layer_{i}'))
    model.add(Bidirectional(LSTM(hp.Choice(f'hidden_layer_{i+1}',values=[32,64]),dropout=0.2),name=f'hidden_layer_{i+1}'))
    '''capa de salida'''
...
resultados =f'{drive}/modelo_cohn_kanade_schdlr/'
datos = f'{drive}/recordings_cohn_kanade/'
